...
| UC-24 | Initialize a repository with existing AI guidance | Inspect and interview first; preserve content; create canonical sources and thin adapters only after preview |
| UC-25 | Edit detects manual changes inside a managed projection | Report drift and ask adopt, replace, or preserve; never overwrite silently |
...
| UC-31 | A managed path crosses a symlink, junction, or reparse point | Both project doctors reject the path before reading or hashing the target |
...
| UC-34 | A JSON property or configured identifier changes casing, or the evidence schema gains an undeclared release field | Python and PowerShell reject it identically; schema field sets are exact and keys, role IDs, artifact paths, and evidence keys are ordinal and case-sensitive |

Acceptance requires:

- structural validation passes for all four skills;
...
- before a major release, forward-test UC-01, UC-03, UC-04, UC-06, UC-08, UC-10, UC-14, UC-18, UC-19, UC-20, UC-23, UC-24, UC-25, and UC-26;
