...
| UC-39 | User asks for durable feature-level functional verification | `$ultracode-verify` creates or manages one closed JSON plan whose scenario histories are append-only and use exactly `planned`, `passed`, `failed`, `not-run`, and `not-applicable`; every criterion resolves and has scenario coverage; passed or failed requires direct status-consistent evidence; planned, not-run, and not-applicable contain no execution evidence; not-run and not-applicable require reasons; malformed, stale, incomplete, contradictory, duplicated, or orphaned content fails closed without overwrite; the derived feature outcome cannot be VERIFIED while applicable coverage is failed, planned, not-run, inconsistent, or missing; plan authority never implies product fixes, Git, publish, external, dependency, destructive, production, or deploy actions; Main UltraCode, Help, Flow, and Status use the same verification semantics |
| UC-40 | User asks to prepare or execute a Git commit | `$ultracode-commit` derives and validates a Conventional Commits 1.0.0 message from the real diff; message-only requests remain read-only; explicit commit authority is scoped to exact paths; unrelated changes stop for a split or narrow-scope decision; a local commit is reported separately from push, release, or deployment |

Acceptance requires:

- structural validation passes for all eight skills;
...
- before a major release, forward-test UC-01, UC-03, UC-04, UC-06, UC-08, UC-10, UC-14, UC-18, UC-19, UC-20, UC-23, UC-24, UC-25, UC-26, UC-35, UC-36, UC-37, UC-38, and UC-39;
...
- forward-test UC-40 proves message-only read-only behavior, exact-path Git authority, mixed-diff split handling, and separate local-commit versus push/release reporting before a release claims the new command is verified.
