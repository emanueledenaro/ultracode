...
| UC-37 | User asks how to use UltraCode or which command fits | `$ultracode-help` stays read-only even in an uninitialized project; explains all six commands in plain language with purpose, when to use, write and confirmation boundaries, result, comparison, and copyable example; supports command, model, and example topics; never initializes, delegates, runs project checks, or invents live state |
